...
namespace NSI_V3
{
    public static class Globals
    {
        ...
        public static string outputFile = "Sudbury-200m1000m"; //output file
        ...
        public static double maxLoop = 50;
        //*********************
    }
    ...
    class NSI_V3
    {
        static void Main()
        {
            ...
            while (looping == 0)
            {
                ...
                /*
                //TREND LOGIC************************
                int[,] trendLOG = new int[lengthX, lengthY]; //Keep track of which cells pass the trending logic as decided by the user input value
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        ...
                        {
                            int orthog = 0;
                            //Find the most isotropic direction, and look at the orthogonal direction
                            if (smallestL[i, j, 0] == 0) //DX
                            {
                                orthog = 1;
                            }
                            else if (smallestL[i, j, 0] == 1) //DY
                            {
                                orthog = 0;
                            }
                            ...
                            double trendRatio = smallestLV[i, j, orthog] / smallestLV[i, j, 0]; //divide the most isotropic direction into its orthogonal direction
                            if (trendRatio < Globals.trendStr) //If the ratio is less than the user-defined trend strength value, then we will not scale that cell
                            {
                                trendLOG[i, j] = 0;
                            }
                            else //If the ratio is greater, than proceed with the scaling
                            {
                                trendLOG[i, j] = 1;
                            }
                        }
                ...
                */
                ...
                for (int i = 0; i < 5; i++)
                {
                    for (int j = 0; j < 5; j++)
                    {
                        double x = i - 2;
                        double y = j - 2;
                        gaus[i, j] = (1 / (2 * Math.Pow(Math.PI * sigma, 2))) * Math.Exp(-((x * x) + (y * y)) / (Math.Pow(2 * sigma, 2)));
                        gTot += gaus[i, j];
                        //counter += 1;
                    }
                }

                //Gaussian Smooth the iMultCells grid
                double[,] iMultCellsS = new double[lengthX, lengthY];
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        ...
                        else
                        {
                            ...
                            gy[3] = j + 1;
                            gy[4] = j + 2;
                            if (i == 0) //Need to also check if -999999!!! or flag = -1.
                            {
                                gx[0] = 0;
                                gx[1] = 0;
                            }
                            ...
                            /*double tempVal = 0;
                            double[] points = new double[9]; // 8 points around center point
                            //Will have to rearrange for now, not following standard method
                            points[1] = iMultCells[gridedData.close1[i, j, 0, 0], gridedData.close1[i, j, 0, 1]];
                            points[2] = iMultCells[gridedData.close1[i, j, 1, 0], gridedData.close1[i, j, 1, 1]];
                            points[5] = iMultCells[gridedData.close1[i, j, 2, 0], gridedData.close1[i, j, 2, 1]];
                            points[8] = iMultCells[gridedData.close1[i, j, 3, 0], gridedData.close1[i, j, 3, 1]];
                            points[7] = iMultCells[gridedData.close1[i, j, 4, 0], gridedData.close1[i, j, 4, 1]];
                            points[6] = iMultCells[gridedData.close1[i, j, 5, 0], gridedData.close1[i, j, 5, 1]];
                            points[3] = iMultCells[gridedData.close1[i, j, 6, 0], gridedData.close1[i, j, 6, 1]];
                            points[0] = iMultCells[gridedData.close1[i, j, 7, 0], gridedData.close1[i, j, 7, 1]];
                            points[4] = iMultCells[i, j];

                            double tempMult = 0;
                            for (int k = 0; k < 9; k++)
                            {
                                tempMult = tempMult + ((gaus[k] / gTot) * points[k]);
                            }
                            iMultCellsS[i, j] = tempMult;*/
                            double tempMult = 0;
                            for (int ik = 0; ik < 5; ik++)
                            {
                                for (int jk = 0; jk < 5; jk++)
                                {
                                    if (iMultCells[gx[ik], gy[jk]] == -999999 || iMultCells[gx[ik], gy[jk]] == -999999) { iMultCells[gx[ik], gy[jk]] = iMultCells[gx[2], gy[2]]; }
                                    tempMult += ((gaus[ik, jk] / gTot) * iMultCells[gx[ik], gy[jk]]);
                                }
                            }
                            iMultCellsS[i, j] = tempMult;
                ...
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        ...
                        {
                            double tempMR = gridedDataDerivIterate.Value[i, j] * iMultCellsS[i, j];
            ...
            //*****************************OUTPUT
            Globals.outputFile = Globals.outputFile + currentLoop + "x.txt";
