...
namespace NaprstekDLL_V19
{
    static class Globals
    {
        ...
        public static int firstTaylorLoop = 5; //how many times the Taylor interpolation should run the initial loop
        public static bool trending = true; //a checkbox of whether or not to do the full trending, or simply Taylor Interpolate
        //*********************
    }
    ...
    public class Execute : BaseForm
    {
        ...
        private static DialogResult ShowInputDialog()
        {
            ...
            textLabel6.Size = new System.Drawing.Size(size.Width - 10, 23);
            inputBox.Controls.Add(textLabel6);

            System.Windows.Forms.TextBox textBox6 = new TextBox();
            textBox6.Size = new System.Drawing.Size(size.Width - 10, 23);
            ...
            //TOTAL LOOP
            Label textLabel7 = new Label() { Left = 5, Top = 170, Text = "Enter # of loops" };
            ...
            textBox7.Text = Convert.ToString(Globals.totalLoop);
            inputBox.Controls.Add(textBox7);

            //TAYLOR LOOP
            Label textLabel8 = new Label() { Left = 5, Top = 225, Text = "Enter # of iterations for the initial Taylor Interpolation" };
            textLabel8.Size = new System.Drawing.Size(size.Width - 10, 23);
            ...
            textBox8.Location = new System.Drawing.Point(5, 248);
            textBox8.Text = Convert.ToString(Globals.firstTaylorLoop);
            inputBox.Controls.Add(textBox8);

            //TRENDING CHECKBOX
            Label textLabel9 = new Label() { Left = 25, Top = 280, Text = "Trending" };
            textLabel9.Size = new System.Drawing.Size(size.Width - 30, 23);
            inputBox.Controls.Add(textLabel9);

            System.Windows.Forms.CheckBox chkbxTrend = new CheckBox();
            chkbxTrend.Location = new System.Drawing.Point(5, 280);
            chkbxTrend.Checked = Globals.trending;
            inputBox.Controls.Add(chkbxTrend);
            ...
            Globals.totalLoop = Convert.ToInt32(textBox7.Text);
            Globals.firstTaylorLoop = Convert.ToInt32(textBox8.Text);
            Globals.trending = chkbxTrend.Checked;
            return result;
        }
        ...
        public static void Main()
        {
            ...
            //****GEOSOFT IMPORTING
            // --- Initialize ---
            pGeo = new CGX_NET("NaprstekDLL_V19", "V1", 2047, 0, 0);
            if (pGeo == null)
            {
            ...
            for (int currentLoop = 0; currentLoop < Globals.totalLoop; currentLoop++)
            {
                CSYS.ProgUpdateL(currentLoop, Globals.totalLoop);
                //****************************DerivV3
                ...
                if (currentLoop == 0)
                {
                    numOfIter = Globals.firstTaylorLoop;
                }

                for (int h = 0; h < numOfIter; h++)
                {
                    for (int i = 0; i < lengthX; i++)
                    {
                        for (int j = 0; j < lengthY; j++)
                ...
                if (Globals.trending == true) //if trending is off, then we ONLY use Taylor Interpolation
                {
                ...
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        ...
                        else //otherwise it is interpolated data, and therefore we must find the real data along lines of isotropy
                        {
                            if (Globals.trending == true)
                            {
                                ...
                                while (multFind == false)
                                {
                                    ...
                                    else //we haven't completed at least one of the directions
                                    {
                                        ...
                                        strength[i, j] = smallSide / totalLV; //find this "strength" identifier, which will generall range between 0 and 0.25, though in more extreme gradient cases it could range higher (techincally up to 0.9)
                            ...
                            else //trending is off, therefore we will normalize without trending, using all 8 directions (if possible).
                            {
                                bool failed = false;
                                double iMult = 0;
                                ...
                                int totDistance = 0;
                                //Check each side for real data cells, and average over all found.
                                for (int k = 0; k < 8; k++)
                                {
                                    if (gridedData.close[i, j, k] != -1)
                                    {
                                        int iD = 0;
                                        int jD = 0;
                                        if (k == 0)
                                        ...
                                        int distance = gridedData.close[i, j, k];
                                        distances.Add(distance);
                                        totDistance = totDistance + distance;
                                        multipliers.Add(multiplierCells[i + (iD * distance), j + (jD * distance)]);
                                    }
                                }

                                if (distances.Count < 2) //not enough real data cells, therefore failure
                                {
                                    failed = true;
                                }
                                else
                                {
                                    //since we will want to use the inverse distance for our weighting, we must first find the total for the inverse distance
                                    double inverseTotDist = 0;
                                    for (int k = 0; k < distances.Count(); k++)
                                    {
                                        double thisMult = totDistance / distances[k];
                                        inverseTotDist = inverseTotDist + thisMult;
                                    }

                                    //now that we have found all of the real data cells, we can find the multiplier we wish to use
                                    double mult = 0;
                                    for (int k = 0; k < distances.Count(); k++)
                                    {
                                        double thisMult = (totDistance / distances[k]) / inverseTotDist;
                                        mult = mult + (thisMult * multipliers[k]); //we want to use all of the inverse distance weighted multipliers of the real cells found
                                    }
            ...
            if (Globals.trending == true)
            {
                ...
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        if (strength[i, j] < minStr)
                        {
                            minStr = strength[i, j];
                        }
                        if (strength[i, j] > maxStr)
                        {
                            maxStr = strength[i, j];
                        }
                    }
                }
                maxStr = maxStr - minStr;
                for (int i = 0; i < lengthX; i++)
                {
                    for (int j = 0; j < lengthY; j++)
                    {
                        strength[i, j] = strength[i, j] - minStr;
                        strength[i, j] = strength[i, j] / maxStr;
                    }
                }
                //
                ...
                for (int loop2 = 0; loop2 < 1; loop2++)
                {
                    ...
                    for (int i = 0; i < lengthX; i++)
                    {
                        for (int j = 0; j < lengthY; j++)
                        {
                            if (realReplace.Flag[i, j] == -1) //If the data is outside of the grid, then don't use it
                            {
                            }
                            else
                            {
                                List<double> points = new List<double>();
                                points.Add(realReplace.Value[gridedData.close1[i, j, 0, 0], gridedData.close1[i, j, 0, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 1, 0], gridedData.close1[i, j, 1, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 2, 0], gridedData.close1[i, j, 2, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 3, 0], gridedData.close1[i, j, 3, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 4, 0], gridedData.close1[i, j, 4, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 5, 0], gridedData.close1[i, j, 5, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 6, 0], gridedData.close1[i, j, 6, 1]]);
                                points.Add(realReplace.Value[gridedData.close1[i, j, 7, 0], gridedData.close1[i, j, 7, 1]]);
                                points.Add(realReplace.Value[i, j]);
                                ...
                                for (int k = 0; k < 9; k++)
                                {
                                    if (points[k] == -999999)
                                    {
                                        points[k] = realReplace.Value[i, j];
                                        gridedData.close1[i, j, k, 0] = i;
                                        gridedData.close1[i, j, k, 1] = j;
                                    }
                                }
                                ...
                                double difference = alphaData.Value[i, j] - realReplace.Value[i, j];
                                ...
                                newDiff = newDiff * (strength[i, j]);
                                realReplace.Value[i, j] = realReplace.Value[i, j] + newDiff;
                            }
                        }
                    }
                }
            ...
            Globals.outputFile = Globals.inputFile + "-" + Globals.cellSize + "m" + Globals.interpDist + "m" + Globals.totalLoop + "x" + Globals.firstTaylorLoop + "i.txt";
            ...
            myfile2.WriteLine("Output grid file (*.grd): " + Globals.inputFile + "-" + Globals.cellSize + "m" + Globals.interpDist + "m" + Globals.totalLoop + "x" + Globals.firstTaylorLoop + "i");
            myfile2.WriteLine("Number of ASCII lines to skip: 16");
            myfile2.WriteLine("Numper of points in each row: " + lengthY);
            myfile2.WriteLine("Number of rows: " + lengthX);
            myfile2.WriteLine("Row orientation: Left bottom to top");
            myfile2.WriteLine("Dummy value: -999999");
            myfile2.WriteLine("X grid point separation: " + Globals.cellSize);
            myfile2.WriteLine("Y grid point separation: " + Globals.cellSize);
            ...
            for (int i = 0; i < lengthX; i++)
            {
                ...
                for (int j = 0; j < lengthY; j++)
                {
                    tempLine = tempLine + realReplace.Value[i, j] + "\t";
                }
